...
public class PotionManager extends Feature {
    ...
    public void update() {
        this.updatePlayer();
        if (HUD.getInstance().isOn() && HUD.getInstance().textRadar.getValue() && Managers.getInstance().potions.getValue()) {            ArrayList<EntityPlayer> removeList = new ArrayList<EntityPlayer>();
    ...
    public String getPotionString(PotionEffect effect) {
        Potion potion = effect.getPotion();
        return I18n.format((String)potion.getName(), (Object[])new Object[0]) + " " + (effect.getAmplifier() + 1) + " " + Potion.getPotionDurationString((PotionEffect)effect, (float)1.0f);
    }
