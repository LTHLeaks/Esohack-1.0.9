...
public class EntityUtil implements Util {
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (EntityUtil.mc.player.getDistanceSq((Entity)player) > range * range || esohack.friendManager.isFriend(player)) continue;
    ...
    public static void swingArmNoPacket(EnumHand hand, EntityLivingBase entity) {
        ...
        if (!entity.isSwingInProgress || entity.swingProgressInt >= EntityUtil.getArmSwingAnimationEnd(entity) / 2 || entity.swingProgressInt < 0) {
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals((Object)EntityUtil.mc.player) || entity instanceof EntityPlayer && esohack.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
